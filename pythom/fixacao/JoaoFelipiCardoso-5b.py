...
def gago(texto):
    ...
    for palavra in texto.split():
        gaguejado += palavra[0] + '-' + palavra + ' '

    return str(gaguejado).strip()
...
def fixa_primeiro(s):
    ...
    return s[0] + s_without_first.replace(s[0], '*')
...
def nomes_pontas(n):
    ...
    return n.upper().split()[0] + ' ' + n.upper().split()[-1]
...
def nomes_pontas_e_iniciais_do_meio(n):
    ...
    nomesSemDe = n.replace(' de', '')

    nomesSemDa = nomesSemDe.replace(' da', '')

    nomes = nomesSemDa.upper().split()
    
    nomes_iniciais = ''

    for i in range(1, len(nomes) - 1):
        nomes_iniciais += nomes[i][0] + ' '

    return nomes[0] + ' ' + nomes_iniciais + nomes[-1]
...
def tres_maiusculas(texto):
    ...
    for i in range(len(texto) - 2):
        if texto[i].isupper() and texto[i+1].isupper() and texto[i+2].isupper():
            return i    
