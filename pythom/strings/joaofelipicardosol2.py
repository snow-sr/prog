...
def palindromo(texto):
    ...
    """
    ini = texto[0:] #armazena a palavra de frente para a trás
    fim = texto[-0:] #armazena a palavra de trás para a frente
    
    return (ini == fim) #verifica se a palavra é igual de trás pra frente
...
def troca_caixa(texto):
    ...
    """
    texto = texto.lower() #deixa todas as letras mínusculas
    texto = texto.replace('a', 'A') #Troca 'a' minusculo por maiusculo
    texto = texto.replace('e', 'E') #Troca 'e' minusculo por maiusculo
    texto = texto.replace('i', 'I') #Troca 'i' minusculo por maiusculo
    texto = texto.replace('o', 'O') #Troca 'o' minusculo por maiusculo
    texto = texto.replace('u', 'U') #Troca 'u' minusculo por maiusculo

    return texto #retorna o texto com as letras trocadas
...
def imprime_mes_por_extenso(data):
    ...
    day, month, year = data.split('/') #Divide os numeros em três variaveis, Day, Month, year
    valuemonth = month #Setou um valor inteiro para o mês
    ...
    #todos os meses em forma de dicíonario, com seu respectivo numero
    res = (day + " de " + months[valuemonth] + " de " + year) #Armazena tudo formatado já com os "Des"

    return res #retorna o texto armazenado 
...
def encontra_caracter(texto, caracter_procurado):
    ...
    return texto.find(caracter_procurado) #Procura pelo Caracter pedido
...
def é_azarado(numero):
    ...
    """
    init = numero[0] #mesma lógica do palindromo, porem este pega apenas *uma* letra, nesse caso a primeira
    fim = numero[-1] #este pega a ultima letra

    return(init == fim) #retorna se o inicio é igual ao fim
...
def maximo(lista):
    ...
    """
    return max(lista) #retorna o máximo da lista
...
def minimo(lista):
    ...
    """
    return min(lista) #retorna o minimo
...
def maior_menor(lista):
    ...
    """
    maior = max(lista) #armazena o maior número da lista
    menor = min(lista) #armazena o menor número da lista
    res = (maior, menor) #armazena os dois em forma de lista
    return res
...
def media_saltos_lista(saltos):
    ...
    """
    soma_pulos = sum(saltos) #somas todos os numeros da lista SALTOS
    return round((soma_pulos) / len(saltos), 1) #faz a média e arredonda
...
def contem(lista, item_procurado):
    ...
    """
    return (item_procurado in lista) #procura um item dentro de uma lista
...
def conta(lista, item_procurado):
    ...
    return lista.count(item_procurado) #conta quantos items tem daquele na lista
...
def mes_extenso(mes):
    ...
    """
    months = [ 'meses', 'jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez'] #meses em forma de tupla
    return months[mes] #retorna o mes
def media_temperaturas(temperaturas):
    ...
    """
    soma_temps = sum(temperaturas) #soma tudo
    return (soma_temps) / len(temperaturas) #faz a média
...
def leet(texto):
    ...
    texto = texto.replace('a', '4') #troca letra pelo seu alfabeto leet
    texto = texto.replace('A', '4') #troca letra pelo seu alfabeto leet

    texto = texto.replace('e', '3') #troca letra pelo seu alfabeto leet
    texto = texto.replace('E', '3') #troca letra pelo seu alfabeto leet

    texto = texto.replace('g', '9') #troca letra pelo seu alfabeto leet
    texto = texto.replace('G', '9') #troca letra pelo seu alfabeto leet

    texto = texto.replace('i', '1') #troca letra pelo seu alfabeto leet
    texto = texto.replace('I', '1') #troca letra pelo seu alfabeto leet

    texto = texto.replace('s', '5') #troca letra pelo seu alfabeto leet
    texto = texto.replace('S', '5') #troca letra pelo seu alfabeto leet

    texto = texto.replace('t', '7') #troca letra pelo seu alfabeto leet
    texto = texto.replace('T', '7') #troca letra pelo seu alfabeto leet

    texto = texto.replace('o', '0') #troca letra pelo seu alfabeto leet
    texto = texto.replace('O', '0') #troca letra pelo seu alfabeto leet
    return texto #retorna o texto já traduzido
...
def apaga(texto, n):
    ...
    Retorna:
        string: o texto convertido, conforme o enunciado.
    """
    apagar = n + 1 #descobre qual letra ele precisa apagar
    texto = texto[0:n] + texto[apagar:] #puxa a palavra até a letra a ser apagada e pula ela
