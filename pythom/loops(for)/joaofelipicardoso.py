...
def soma_das_temperaturas(lista):
    ...
    """
    soma = 0       #Definimos um Contador inicial, realiza a função de contar quantas vezes a gente dá loops pela lista
    for item in lista:  #Aqui definimos a Lista onde iremos analisar os dados, já passamos tambem a variavel da lista (items)
        soma += item  
    return soma
...
def quantidade_de_impares(valor_inicial, valor_final):
    ...
    par = impar = 0 #Define uma váriavel para o par e o impar
...
def soma_das_temperaturas_dos_inteiros(valor1, valor2):
    ...
    soma = 0
    for item in range(valor1, valor2):
        if item > 0:
            soma += item
        else:
...
def intercalamento_listas(lista1, lista2):
    ...
    """
    list_res = [lista1, lista2]
    saida_list = [None] * len(list_res[0]) * len(list_res)
...
def dar_troco(valor_a_pagar, valor_em_dinheiro):
    ...
    """
    troco = []
    pagar = valor_em_dinheiro - valor_a_pagar
    notas = [50, 20, 10, 5, 2, 1]
    
    for item in notas:
        qntsnotas = pagar//item
    
        if pagar/item >= 1:
            troco.append((item, qntsnotas))
            pagar -= (item*qntsnotas)
...
def media_anual(temperaturas):
    ...
    for items in temperaturas:
        if items > media:
            new_temps.append(count)
        count += 1
    else:
        return new_temps    
...
def maiores_13(idades, alturas):
    ...
    for i in range(len(idades)):
        if idades[i] >= 13:
            if alturas[i] < round(media, 2):
                new_alts.append(alturas[i])
    return new_alts
...
def altera_salarios(salarios):
    ...
    sm = 724.00
    lista = []
    for items in salarios:
        if items <= sm:
            salary = items + (items/100 * 20)
            lista.append(salary)
        elif items >= sm and items <= sm*2:
            salary = items + (items/100 * 15)
            lista.append(salary)
        elif items >= sm*2 and items <= sm*5:
            salary = items + (items/100 * 10)
            lista.append(salary)
        elif items >= sm*5:
            salary = items + (items/100 * 5)
            lista.append(salary)
    return lista
