...
def tempo_para_percorrer_uma_distancia(distancia, velocidade):
    ...
    """        
    velmedia = (distancia / velocidade)
    return round(velmedia, 2)
...
def aumento_salarial(salario, porcentagem):
    ...
    """
    valoradicional = (porcentagem) / 100
    novosalario = (salario) * valoradicional
    salariofinal = novosalario + salario
...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    """
    desconto = (percentual_desconto * preco_original) / 100
...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    daytosecond = (dias * 86400)
    hourtosecond = (horas * 3600)
    minutetosecond = (minutos * 60)    

    segundos_convertidos = (daytosecond) + (hourtosecond) + (minutetosecond) + (segundos)
    return round(segundos_convertidos,1)
...
def fahrenheit_para_celsius(farenheit):
    ...
    """
    farentocelsius = (farenheit - 32) * 5/9
    return round(farentocelsius,2)
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """ 
    MinutesLostBySmoking = anos_fumando * 365 * cigarros_fumados_por_dia * 10
    DaysLostBySmoking = MinutesLostBySmoking / (60*24)

    return round(DaysLostBySmoking, 2) 
...
def dois_elevado_a_um_milhao():
    ...
    """
    toMillion = str(2 ** 1000000)
    len(toMillion)
    return(len(toMillion))
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    """
    parcial1 = (p1 * 7) + (ep1 * 3) / (7+3)
    parcial2 = (p2 * 7) + (ep2 * 3) / (7+3)
...
def salario(valor_hora, horas_mensais):
    ...
    """
    Salary = (valor_hora * horas_mensais)
    discountOnSalary = (Salary * 24) / 100
    SalaryFinal = (Salary - discountOnSalary)
    return int(SalaryFinal)
...
def tinta(metros_pintar):
    ...
    minInk = 0.40
    ink = 18 * 3
    result = (metros_pintar / ink) + minInk
    return round(result)
...
def decompor_numero(numero):
    ...
    unit = numero % 10
    numero = (numero - unit)/10

    dozen = numero % 10

    numero = (numero - dozen)/10
    Hundred = numero
    ...
    SetofNumbers = (Hundred, dozen, unit)
